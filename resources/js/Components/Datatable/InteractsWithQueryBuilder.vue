...
export default {
  ...
  data() {
    const queryBuilderData = {
      page: this.datatable.page || 1,
      sort: this.datatable.sort || "",
      filter: this.getFilterForQuery(
        this.datatable.search || {},
        this.datatable.filters || {}
      ),
      columns: this.getColumnsForQuery(this.datatable.columns || {}),
    };
  ...
  methods: {
    ...
    _headerCellData(columnKey, sortable) {
      ...
      if (this.datatable.columns) {
        const columnData = this.datatable.columns[columnKey];
