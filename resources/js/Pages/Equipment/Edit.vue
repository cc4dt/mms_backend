<template>
  <app-layout :title="category.name">
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{category.name}} Edit</h2>
    </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    function submit(event) {
      if (event.submitter.id == "add") this.addProcesses();
        ...
        console.log(data);
        Inertia.put(route("maintenance." + this.category.slug + ".update", this.maintenance.id), data, {
          // forceFormData: true,
          onError: (errors) => {
            alert(errors.create);
  ...
  created() {
    var processes = [];
    this.maintenance.processes.forEach((process) => {
      var mf = this.category.forms.find((e) => {
        return e.equipment_id == process.equipment_id;
      });
      if (mf) {
        var procedures = {};

        mf.procedures.forEach((element) => {
          ...
          procedures[element.id] = {
            ...
            spare: element.spare_parts?.find((e) => {
              return e.id == detail?.spare_part?.id;
            }),
            val: detail?.value,
          };
        });
        ...
        processes.push({
          id: process.id,
          mf: mf,
          equipment: process.master_equipment,
          description: process.description,
          procedures: procedures,
        });
      }
    });
    this.form.id = this.maintenance.id;
    this.form.station = this.stations?.find((e) => {
      return e.id == this.maintenance?.station_id;
    });
    this.form.date = new Date(this.maintenance?.date).toISOString().slice(0, 10);
