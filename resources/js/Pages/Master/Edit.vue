<template>
  <app-layout title="Edit Equipment">
    <template #header>
...
export default defineComponent({
  ...
  methods: {
    ...
    submit(event) {
      ...
      Object.keys(this.form.details).forEach((key) => {
        data.details.push({
          attribute_id: key,
          value: this.form.details[key] ?? null,
        });
      });
      console.log(data);
      Inertia.put(route("master.update", this.master.id), data, {
        // forceFormData: true,
        onError: (errors) => {
          alert(errors.update);
  ...
  created() {
    this.form.serial = this.master.serial;
    this.form.station = this.stations.find((e) => {
      return e.id == this.master.station_id;
    });
    this.form.equipment = this.equipment.find((e) => {
      return e.id == this.master.equipment_id;
    });
    
    this.form.details = {};
    this.form.equipment?.attributes?.forEach((element) => {
      this.form.details[element.id] = this.master.details.find((e) => {
        return e.attribute_id == element.id;
      })?.value;
    });
  },
});
